...
def run_clustering(algo_params, num_clusters, input_df):
    ...
    if algo_params['algorithm'] == 'hierarchical':
        for linkage in base_params['linkage']:
                ...
                run_cluster(output_path, input_path,input_df, **params)
...
for num_clusters in list_clusters:
    ...
    if run_gmm:
        print('GMM')
        run_clustering(gmm_params, num_clusters, input_df)
    if run_hc:
        print('HC')
        run_clustering(hierarchical_params, num_clusters, input_df)
    if run_km:
        print('KM')
        run_clustering(kmeans_params, num_clusters, input_df)
